...
- 验证日期：2026 年 6 月 22 日。
- 验证环境：Python 3.13.5。
- `python3 -m compileall -q backend`：通过。
- `python3 -m unittest discover -s backend/tests -v`：共 73 项，全部通过。
- 当前环境使用 Pydantic 2.x，测试输出会提示部分旧式 `Config` 键的兼容性警告；警告不影响本次测试结果，但后续可单独迁移相关模型配置。
